...
export default function filter(state = initialState, action) {
    ...
    switch (action.type) {
        case CHOOSE_GROUP:
            return {...state, inSearch: true, searchError: false, founded: {count: 0, items: []}, stepDiff: 1, canLoadMore:true};
        case START_SEARCH:
            return {...state, inSearch: true, searchError: false, founded: {count: 0, items: []}, stepDiff: 1, canLoadMore:true};
        case SEARCH_ERROR:
            return {...state, inSearch: false, searchError: true, founded: {count: 0, items: []}, canLoadMore:true};
        case SEARCH_PART:
            return {...state, inSearch: false, searchError: false, founded: action.payload, offset: 0, stepDiff: 2, canLoadMore:true};
        case SEARCH_PART_NEXT:
            let canLoadMore = action.stepDiff < 4;
            return {
                ...state,
                inSearch: false,
                searchError: false,
                founded: mergeFounded(state.founded, action.payload),
                stepDiff: action.stepDiff + 1,
                canLoadMore: canLoadMore
            };
        case SET_FILTER:
            return Object.assign({}, state, action.payload, {inSearch: true, searchError: false, founded: {count: 0, items: []}, stepDiff: 1, canLoadMore:true});
            break;
        case LIKE:
